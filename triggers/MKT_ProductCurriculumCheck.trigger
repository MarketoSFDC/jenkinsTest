trigger MKT_ProductCurriculumCheck on lmscons__Curriculum__c (before insert, before update) {
	Set<Id> ProductIdsSet = new Set<Id>();
	Map<Id, List<lmscons__Curriculum__c>> ProductIdsListObjectsMap = new Map<Id, List<lmscons__Curriculum__c>>();
	Map<Id, List<lmscons__Curriculum__c>> ProductIdsListObjectsMapOld = new Map<Id, List<lmscons__Curriculum__c>>();
	Map<Id, Id> ProductIdsObjectsIdMapOld = new Map<Id, Id>();
	if (Trigger.isInsert) {

		for (lmscons__Curriculum__c NewCourse : trigger.new) {
			if (NewCourse.Product__c != NULL) {
				List<lmscons__Curriculum__c> CourseList;
				if (ProductIdsListObjectsMap.containsKey(NewCourse.Product__c)) {
					CourseList = ProductIdsListObjectsMap.get(NewCourse.Product__c);
				}
				else {
					CourseList = new List<lmscons__Curriculum__c>();
				}
	...
	if (Trigger.isUpdate) {
		...
		for (lmscons__Curriculum__c OldCourse : trigger.old) {
		...
		if (ProductsIdsSet.size() > 0) {
			...
			for(Id ProdId : ProductsIdsList) {
				if(ProductIdsListObjectsMapOld.containsKey(ProdId)) {
					for (lmscons__Curriculum__c Coursetem : ProductIdsListObjectsMapOld.get(ProdId)) {
		...
		if (ProductsIdsSet.size() > 0) {
			...
			for(Id ProdId : ProductsIdsList) {
				if(ProductIdsListObjectsMap.containsKey(ProdId)) {
					for (lmscons__Curriculum__c Coursetem : ProductIdsListObjectsMap.get(ProdId)) {
